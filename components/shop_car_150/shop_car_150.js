// components/shop_car_150/shop_car_150.js
...
Component({
    ...
    methods: {
        ...
        selectedOne(e) {
            ...
            if (flag) {
                if (this.data.selectList.length == 6){
                    this.marked.show({
                        mode: 'error',
                        msg: '最多选择6个'
                    })
         ...
         */
        success() {
            let lastList = getApp().globalData.shop_car_150
            ...
            getApp().globalData.shop_car_150 = newList
        ...
        formatData(data) {
            ...
            for (let key in data) {
                ...
                data[key].supply_price = '150.00'
                data[key].int_price = '150'
                data[key].decimal = '00'
                arr.push(data[key])
            }
            return arr
        ...
        getShopCarWithPrice() {
            ...
            Request.Ajax(Url.getShopCarWithPrice(), params, 'POST').then(res => {
                console.log(res)
                if (res.code == 200) {
                    getApp().globalData.shop_car_bool['150.00'] = true
                    if (res.data instanceof Array) {
                        _this.setData({
                            state: 'noMore'
                        })
                    } else {
                        getApp().globalData.shop_car_150 = _this.formatData(res.data).sort(_this.compare())
                        _this.getPageOne()
                    }
                ...
                console.log(_this.formatData(res.data).reverse())
            }).catch(res => {
                // _this.load.hide()
                _this.marked.show()
                _this.setData({
        ...
        getPageOne() {
            ...
            this.loadMore = this.selectComponent('#loadMore')
            this.loadMore.setState('Loading')
            let ls = getApp().globalData.shop_car_150;
            console.log(ls)
        ...
        init() {
            this.loadMore = this.selectComponent('#loadMore')
            this.marked = this.selectComponent('#marked')
            this.laod = this.selectComponent('#load')
            let ls = getApp().globalData.add_car_list
            console.log(ls)
            if (getApp().globalData.shop_car_bool['150.00']) {
                if (ls.includes('150.00')) {
                    this.setData({
                        page: 1,
                        shopList: null
                    })
                    this.getPageOne()
                    getApp().globalData.add_car_list = this.removeArray(ls, '150.00')
                }
            } else {
